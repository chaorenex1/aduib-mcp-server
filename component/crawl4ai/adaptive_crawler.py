...
class CrawlState:
    ...
    def load(cls, path: Union[str, Path]) -> 'CrawlState':
        ...
        state.documents_with_terms = defaultdict(set,
                                                 {k: set(v) for k, v in state_dict['documents_with_terms'].items()})
        ...
        state.kb_embeddings = np.array(state_dict['kb_embeddings']) if state_dict.get(
            'kb_embeddings') is not None else None
        state.query_embeddings = np.array(state_dict['query_embeddings']) if state_dict.get(
            'query_embeddings') is not None else None
...
class AdaptiveConfig:
    ...
    def validate(self):
        ...
        assert abs(
            self.embedding_nearest_weight + self.embedding_top_k_weight - 1.0) < 0.001, "Embedding weights must sum to 1"
...
class EmbeddingStrategy(CrawlStrategy):
    ...
    async def map_query_semantic_space(self, query: str, n_synthetic: int = 10) -> Any:
        ...
        variations = {'queries': ['what are the best vegetables to use in fried rice?',
    ...
    async def select_links_for_expansion(
        ...
        from crawl4ai.utils import cosine_distance, get_text_embeddings
        ...
        if texts_to_embed:
            ...
            new_embeddings = await get_text_embeddings(texts_to_embed, self.llm_config, self.embedding_model)
    ...
    async def should_stop(self, state: CrawlState, config: AdaptiveConfig) -> bool:
        ...
        min_confidence_threshold = config.embedding_min_confidence_threshold if hasattr(config,
                                                                                        'embedding_min_confidence_threshold') else 0.1
        ...
        min_relative_improvement = self.config.embedding_min_relative_improvement * confidence if hasattr(self,
                                                                                                          'config') else 0.1 * confidence
    ...
    async def update_state(self, state: CrawlState, new_results: List[CrawlResult]) -> None:
        ...
        if hasattr(state, 'query_embeddings') and state.query_embeddings is not None:
            state.coverage_shape = self.compute_coverage_shape(state.query_embeddings,
                                                               self.config.alpha_shape_alpha if hasattr(self,
                                                                                                        'config') else 0.5)
                    ...
                    for result, (link, _) in zip(new_results, to_crawl):
                        if result:
                            ...
                            if hasattr(result, 'links') and result.links:
                                ...
                                if isinstance(result.links, dict):
                                    # Extract internal and external links from dict
                                    internal_links = [Link(**link_data) for link_data in
                                                      result.links.get('internal', [])]
                                    external_links = [Link(**link_data) for link_data in
    ...
    def print_stats(self, detailed: bool = False) -> None:
        ...
        if not detailed and use_rich:
            ...
            if isinstance(self.strategy, EmbeddingStrategy):
                ...
                table.add_row("", "")  # Spacer
                table.add_row("Is Sufficient?",
                              "[green]Yes (Validated)[/green]" if self.is_sufficient else "[red]No[/red]")
            else:
    ...
    def _crawl_result_to_export_dict(self, result) -> Dict[str, Any]:
        ...
        if self.state:
            export_dict['crawl_metadata'] = {
                'crawl_order': self.state.crawl_order.index(export_dict['url']) + 1 if export_dict[
                                                                                           'url'] in self.state.crawl_order else 0,
